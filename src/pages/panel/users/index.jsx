...
import { Button, Spinner, Table, Modal, SearchBox } from "../../../components";
...
import { clearEditInfo, clearInfo } from "../../../redux/reducers/user-company";
import {
  deleteUserInfo,
  editUserInfo,
  getByIdUserInfo,
...
export const UsersList = () => {
  // --------- store --------
  const { editInfo } = useSelector((state) => state.userInfoSlice);
  const { info } = useSelector((state) => state.userInfoSlice);
  const { isLoading: loading } = useSelector((state) => state.loadingSlice);

  // --------- states --------
  const [data, setData] = useState();
  const [openModal, setOpenModal] = useState(false);
  const [modalType, setModalType] = useState({ type: "", data: null });
        ...
        <div>
          <div className="flex gap-2 items-center">
            ...
            <div className="h-full flex flex-col gap-2 justify-between">
              <p className="capitalize font-semibold">{row.userName}</p>
              ...
              {action.type === "add" && (
                <>
                  <div
                    ...
                    onClick={() => {
                      setOpenModal(true);
                      setModalType({ type: "companies", data: row.id });
                      setUserInfo(`${row.firstName} ${row.lastName}`);
                    }}
                  >
                ...
                <>
                  <div
                    ...
                    onClick={() => {
                      if (!row.isActive) {
                        setShouldEdit(true);
                        dispatch(getByIdUserInfo(row.id));
                      }
                    }}
                  >
                    <TbProgressCheck
  ...
  useEffect(() => {
    if (Object.keys(editInfo).length > 0 && shouldEdit) {
      dispatch(
        editUserInfo(
          editInfo.userInfoId,
          {
  ...
  const openModalHandler = (modal, id) => {
    setOpenModal(true);
    setModalType({ type: modal, data: id });
  };
  ...
  return (
    <>
      ...
      >
        ...
        {modalType.type === "edit" && (
          <EditUser
            editId={modalType.data}
            onCloseModal={closeModalHandler}
            isReloadPage={() => reloadPageHandler((currState) => !currState)}
          />
        )}
        {modalType.type === "companies" && (
          <UserCompanies
            userId={modalType.data}
            userName={userInfo}
            onCloseModal={closeModalHandler}
            isReloadPage={() => reloadPageHandler((currState) => !currState)}
          />
