...
import { Table, Spinner, Button, Modal } from "../../../../components";
import { AddCompany } from "./add_company";

export const UserCompanies = ({ userId, userName, onCloseModal }) => {
  // --------- store --------
  ...
  const cols = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="dark:text-dark_custom-full-white flex">
          {row.isActive ? (
            <span className="bg-green-100 p-1 rounded-lg text-green-500">
              Active
            </span>
          ) : (
            <span className="bg-red-50 rounded-lg p-1 text-red-500">
              Not active
            </span>
          )}
        </div>
    ...
    {
      name: t("table.col.no_of_employees"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">
    ...
    //                   color="gray"
    //                   className={!row.isActive ? "opacity-100" : "opacity-20"}
    //                   size={18}
    //                 />
    //               </div>
    //               {!row.isActive && (
    //                 <Tooltip id="activate">{t("tooltip.activate")}</Tooltip>
    //               )}
    //             </>
    //           )}
    ...
    //             <div
    //               key={index}
    //               className={`p-2 rounded-md bg-custom-gray-light ${
    //                 row.isActive ? "cursor-pointer" : "cursor-default"
    //               }`}
    //               onClick={() => dispatch(getByIdUserCompany(row.id))}
    //             >
    //               <svg
    //                 width="16"
    //                 height="16"
  ...
  useEffect(() => {
    if (info.length) {
      const values = info.map((item, index) => ({
        ...
        numberOfEmployees: item.company?.numberOfEmployees,
        establishmentYear: item.company?.establishmentYear,
      }));
      setData(values);
    } else setData({});
  }, [info]);
  useEffect(() => {
    if (Object.keys(editInfo).length) {
      const edited = {
        ...editInfo,
        siteUrl: editInfo.siteUrl == null ? "" : editInfo.siteUrl,
        isActive: !editInfo.isActive,
      };
      dispatch(
        editUserCompany(editInfo.userCompanyId, edited, (state) =>
  ...
  return (
    <>
      <Modal
        state={modalType.type == "add"}
        onCloseModal={() => setModalType({ type: "" })}
      >
        <AddCompany
          onCloseModal={() => setModalType({ type: "" })}
          userCompanyList={info}
          userId={userId}
          userName={userName}
      ...
      <div className="dark:bg-dark_custom-light-black rounded-10 overflow-hidden">
        <div className="flex flex-col gap-y-2 w-full p-6 pb-4 bg-white dark:bg-dark_custom-light-black">
          <div className="flex justify-between items-center dark:bg-dark_custom-light-black">
            <div className="flex flex-col gap-y-1">
              ...
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted dark:text-dark_custom-light-white">
                {t("text.users")} - {t("text.companies_list")}
              </h4>
            </div>
          ...
          <div>
            <div className="flex justify-between pt-[20px] border-t">
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted dark:text-dark_custom-light-white">
                {t("table.result")}:
              ...
              <Button
                title={t("button.add_company")}
                classes={"-mt-2"}
                onClick={() => setModalType({ type: "add", data: null })}
              />
            </div>
          </div>
