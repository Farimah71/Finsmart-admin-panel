...
export const Definition = () => {
  ...
  const { info: definitionData } = useSelector(
    (state) => state.definitionSlice
  );
  const { info: definitionTypeData } = useSelector(
    (state) => state.definitionTypeSlice
  );
  ...
  const [defintionTypeOptions, setDefinitionTypeOptions] = useState();
  ...
  const cols = [
    {
      name: t("table.col.no"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white flex">{row.rowId}</div>
      ),
    },
    {
      name: t("table.col.title"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white flex">{row.title}</div>
      ),
    },
    {
      name: t("table.col.description"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white flex">{row.desc}</div>
      ),
    },
    {
            ...
            <>
              ...
              {action.type === "delete" && (
                <div
                  key={index}
                  className="p-2 rounded-md bg-custom-gray-light cursor-pointer"
                  onClick={() => dispatch(deleteHandler(row.id))}
                >
                  <svg
                    width="16"
                    height="16"
  ...
  // --------- lifecycle ----------
  useEffect(() => {
    !Object.keys(definitionData).length &&
      setMessage(t("text.select_definition_type"));
  ...
  useEffect(() => {
    if (definitionTypeData.count) {
      ...
      setDefinitionTypeOptions(definitionTypes);
    }
  }, [definitionTypeData]);
  useEffect(() => {
    if (definitionData.count) {
      const values = definitionData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.definationId,
        title: item.title,
      ...
      setData();
      Object.keys(definitionData).length && setMessage(t("text.no_definition"));
    }
  }, [definitionData]);
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.SettingsApi.deleteDefinition + id)
      .then((res) => {
  ...
  const filterByTypeHandler = (value) => {
    value &&
      dispatch(
        getDefinitions({
          filters: [
  ...
  return (
    <>
      ...
      <div className="dark:bg-dark_custom-light-black rounded-10">
        <div className="flex flex-col gap-y-8 w-full p-6 pb-4 rounded-10 bg-white dark:bg-dark_custom-light-black">
            ...
            <div className="flex justify-between pt-[30px] border-t">
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted dark:text-dark_custom-light-white">
                ...
                <span className="text-custom-gray-200 font-bold dark:text-dark_custom-light-white">
                  {definitionData.count}
                </span>
              </h4>

              <div className="flex gap-x-2 lg:w-2/6">
                <MyForm initialValues={{}} classes="dark:bg-transparent">
                  <div>
                    <Field
                      component={SelectBox}
                      options={defintionTypeOptions}
                      name="definitionType"
                      onChangeHandler={(value) =>
                        filterByTypeHandler(value.value)
                      }
                      placeholder={t(
                        "input.select_definition_type.placeholder"
                      )}
                      disabled={!definitionTypeData.count}
                    />
                  </div>
                </MyForm>
                <Button
          ...
          <span className="text-custom-blue text-center">{message}</span>
