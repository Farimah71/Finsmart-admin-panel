...
import {
  editDefinitionType,
  getByIdDefinitionType,
} from "../../../../redux/actions/settings/definition-type";
import { Field } from "formik";
import * as Yup from "yup";

export const EditDefinitionType = ({ editId, onCloseModal, isReloadPage }) => {
  // ---------- store ----------
  ...
  const onSubmit = (values) => {
    dispatch(
      editDefinitionType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycles ----------
  useEffect(() => {
    dispatch(getByIdDefinitionType(editId));
  }, []);
  useEffect(() => {
    if (Object.keys(editInfo).length > 0) {
      setFormValue({
        definationTypeId: editId,
    ...
    <>
      <div className="flex items-center justify-between p-4 select-none border-b border-custom-gray-light dark:bg-dark_custom-average-black rounded-t-10">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white rounded-10">
          {t("page_title.edit_definition_type")}
        </h4>
        <div
