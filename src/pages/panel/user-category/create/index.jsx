...
import { createUserCategory } from "../../../../redux/actions/settings/user-category";
import * as Yup from "yup";

export const CreateUserCategory = ({ onCloseModal, isReloadPage }) => {
  // ---------- store ----------
  const { loading } = useSelector((state) => state.userCategorySlice);
  ...
  const onSubmit = (values) => {
    dispatch(createUserCategory(values, (status) => reloadPageHandler(status)));
  };
    ...
    <>
      <div className="flex items-center justify-between p-4 select-none border-b border-custom-gray-light dark:bg-dark_custom-average-black rounded-t-10">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white rounded-10">
          {t("page_title.create_user_category")}
        </h4>
        <div
