...
import { Button, Table, Modal, Spinner } from "../../../../components";
import { useDispatch, useSelector } from "react-redux";
import { getFinsmartCodingHeaders } from "../../../../redux/actions/settings/Finsmart-coding-header";
import { useNavigate } from "react-router-dom";
import { CreateAccountPlanning } from "./form/create";
import { EditAccountPlanning } from "./form/edit";
...
export const AccountPlanning = () => {
  ...
  // ---------- store ----------
  const { loading } = useSelector((state) => state.FinsmartCodingHeaderSlice);
  const { info: codingHeaderData } = useSelector(
      ...
      cell: (row) => (
        <div className="group relative flex items-center justify-center gap-x-2">
          ...
          <div
            className="p-2 rounded-md bg-custom-gray-light cursor-pointer"
            onClick={() =>
              navigate("/accountPlanning/detail", { state: row.id })
            }
          >
            <TbListDetails size={17} className="opacity-40" />
          </div>
        </div>
      ),
    },
  ...
  useEffect(() => {
    if (codingHeaderData) {
      const values = codingHeaderData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.finsmartCodingHeaderId,
        title: item.title,
      }));
      setData(values);
    }
  ...
  useEffect(() => {
    dispatch(getFinsmartCodingHeaders({ filters: [] }));
  }, [isReload]);

  // -------- functions --------
  const deleteHandler = (id) => {
    axios
      .delete(api.SettingsApi.deleteFinsmartCodingHeader + id)
      .then((res) => {
  ...
  return (
    <>
      ...
      <div className="dark:bg-dark_custom-light-black rounded-10 overflow-hidden">
        <div className="flex flex-col gap-y-2 w-full p-6 bg-white dark:bg-dark_custom-light-black">
          <div className="flex justify-between items-center flxco dark:bg-dark_custom-light-black">
            <div className="flex gap-y-1">
              <div className="flex flex-col gap-y-1 mb-6">
                <h4 className="text-19 text-custom-dark font-bold dark:text-dark_custom-full-white">
                  {t("page_title.account_planning")}
                </h4>
          ...
          {loading && !data ? <Spinner /> : <Table cols={cols} data={data} />}
        </div>
      </div>
    </>
  );
